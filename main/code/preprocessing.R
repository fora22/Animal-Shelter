...
# 단계선택법
# df[,50:52]
# 전체유기동물수 <-df[,50] + df[,51] + df[, 52]
# abandoned_animal <- data.frame(전체유기동물수)
# human <- cbind(human, abandoned_animal)
# head(human)

# scale_human <- scale(human, center=TRUE)
# class(scale_human)
# scale_human <- data.frame(scale_human)
# model_h <- lm(전체유기동물수 ~ 남자..명. +여자..명. + 단독주택.계 + 아파트 + 연립주택 + 
#                      다세대주택 + 세전.월소득 + 관리자 + 기능원.및.관련기능종사자 + 기타 + 
#                      농림어업.숙련.종사자 + 단순노무종사자 + 사무종사자 + 서비스종사자 + 
#                      장치기계조작.및.조립종사자 + 전문가.및.관련종사자 + 판매종사자 +
#                      X1인가구 + X2인가구 + X3인가구 + X4인가구 + X5인가구 + X6인가구 + 
#                        X7인이상.가구 + X0.4세 + X5.9세 + X10.14세 + X15.19세 + X20.24세 + 
#                        X25.29세 + X30.34세 + X35.39세 + X40.44세 + X45.49세 + X50.54세 + 
#                        X55.59세 + X60.64세 + X65.69세 + X70.74세 + X75.79세 + X80.84세 + X85.89세 +
#                        X90.94세 + X95.99세 + X100세.이상, data = scale_human)
# model_h <- lm(전체유기동물수 ~ ., data = scale_human)
# summary(model_h)

# step(model_h, direction = "both")
